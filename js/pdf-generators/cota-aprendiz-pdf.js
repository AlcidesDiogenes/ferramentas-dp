...
function exportarPdfCotaAprendiz() {
    ...
    const pdfMakeObj = window.pdfMake || (typeof pdfMake !== 'undefined' ? pdfMake : null);

    if (!pdfMakeObj) {
        if (window.showToast) {
            window.showToast("A biblioteca de geração de PDF (pdfMake) ainda não foi carregada. Por favor, aguarde alguns segundos e tente novamente.", "error");
        } else {
            alert("A biblioteca de geração de PDF (pdfMake) ainda não foi carregada.");
        }
        return;
    }

    // Garantir que a tabela de fontes vfs está conectada
    if (!pdfMakeObj.vfs && window.pdfMake && window.pdfMake.vfs) {
        pdfMakeObj.vfs = window.pdfMake.vfs;
    }
    ...
    const buildTableBody = (dataArray, colunas) => {
        ...
        dataArray.forEach(row => {
            const dataRow = [];
            colunas.forEach(c => {
                let valor = row[c.key] !== undefined && row[c.key] !== null ? row[c.key].toString() : "";
                dataRow.push({ 
                    text: valor, 
                    alignment: c.alignment || 'left', 
        ...
        content: [
                ...
                table: {
                    ...
                    body: [
                        [
                            {
                                text: [
                                    { text: 'Empresa: ', bold: true, color: '#334155' }, `${dados.empresa ? dados.empresa.razaoSocial : 'Não informada'}\n`,
                                    { text: 'CNPJ: ', bold: true, color: '#334155' }, `${dados.empresa ? dados.empresa.cnpj : 'Não informado'}`
                                ],
                                fillColor: '#f8fafc',
            ...
            { text: '2. DIMENSIONAMENTO DA COTA OBRIGATÓRIA', style: 'sectionTitle' },
            {
                table: {
                    ...
                    body: [
                        ...
                        [
                            { text: (dados.dimensionamentoCotas.cotaMinima || 0).toString(), alignment: 'center', fontSize: 24, bold: true, color: '#10b981', margin: [0, 15, 0, 15] },
                            { text: (dados.dimensionamentoCotas.cotaMedia || 0).toString(), alignment: 'center', fontSize: 24, bold: true, color: '#0ea5e9', margin: [0, 15, 0, 15] },
                            { text: (dados.dimensionamentoCotas.cotaMaxima || 0).toString(), alignment: 'center', fontSize: 24, bold: true, color: '#f59e0b', margin: [0, 15, 0, 15] }
                        ]
                    ]
                },
            ...
            { text: '3. BASE DE CÁLCULO LÍQUIDA (ELEGÍVEIS)', style: 'sectionTitle', pageBreak: 'before' },
            {
                table: {
                    ...
                    body: buildTableBody(dados.detalhes ? dados.detalhes.base : [], [
            ...
            { text: '4. FUNÇÕES EXCLUÍDAS DA COTA', style: 'sectionTitle' },
            {
                table: {
                    headerRows: 1,
                    widths: ['auto', '*', 'auto', 'auto'],
                    body: buildTableBody(dados.detalhes ? dados.detalhes.excluidos : [], [
                        { text: 'CBO', key: 'cbo', alignment: 'center' },
                        { text: 'Título da Função', key: 'titulo' },
    ...
    pdfMakeObj.tableLayouts = pdfMakeObj.tableLayouts || {};
    pdfMakeObj.tableLayouts.stripedStyle = {
    ...
    try {
        ...
        pdfMakeObj.createPdf(docDefinition).download(`Relatorio_Aprendiz_${cnpjNome || 'geral'}.pdf`);
        if (window.showToast) {
        ...
        if (window.showToast) {
            window.showToast("Erro ao gerar relatório PDF: " + err.message, "error");
        } else {
...
function initCotaAprendizPdf() {
    const btnPdf = document.getElementById("btn-exportar-pdf");
    if (!btnPdf) return;

    btnPdf.removeEventListener("click", exportarPdfCotaAprendiz);
    btnPdf.addEventListener("click", exportarPdfCotaAprendiz);
}

if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", initCotaAprendizPdf);
} else {
    initCotaAprendizPdf();
}
